...
total_num_images = len(os.listdir('/home/ubuntu/inzamam/videoframes')) - 1
...
for i in range(num_batch):
    output_dict = sess.run(tensor_dict, feed_dict = {image_tensor: get_batch(start = i * batch_size, batch_size = batch_size,
...
if len(TEST_IMAGE_PATHS) % batch_size != 0:
    ...
    output_dict = sess.run(tensor_dict, feed_dict= {image_tensor: get_batch(start = (i + 1) * batch_size, end = (i + 1) * batch_size +                                                                                 remaining_batch, im_width=im_width, im_height=im_height,
