...
def main(args):
    ...
    if args.mode == 'video':
        ...
        for js in tqdm.tqdm(json_file):
            ...
            directionAngles = []
            for p in pose.reshape(-1,3)[1:]:
            	directionAngles.append(direction_angles(p))

            ppHip = points_proj[0]
            final_pose = []
            final_pose.append([ppHip[0], ppHip[1], 0])
            for i, pp in enumerate(points_proj[1:]):
            	bMode = np.linalg.norm(np.array(pp) - np.array(ppHip))
            	xAngles = directionAngles[i]
            	nx = xAngles[0] * bMode
            	ny = xAngles[1] * bMode
            	nz = xAngles[2] * bMode
            	final_pose.append([nx+ppHip[0],ny+ppHip[1],nz])
            '''
            if first_frame:
                #scale = np.abs(final_pose[6][1] - final_pose[10][1]) / 1.7018
                scalepp = 1.7018 / np.linalg.norm(np.array(points_proj[6]) - np.array(points_proj[10]))
                print("scalepp value", scalepp)
                first_frame = False
                prev_hip = points_proj[0]
                continue

            x_trans = np.linalg.norm(np.array(points_proj[0]) - prev_hip) * scalepp
            x_trans_array.append(x_trans)
#             print(x_trans)
            prev_hip = points_proj[0]

            pose.T[0::3] += prev_hip_loc[0]
            pose.T[1::3] += prev_hip_loc[1]
            pose.T[2::3] += prev_hip_loc[2]

            pose.T[0::3] += x_trans
            prev_hip_loc = pose.reshape(-1,3)[0]
            
#             print("Prev_hip", prev_hip)
#             print("points_proj", points_proj)
#             print("pose", pose)
#             print("Prev_hip_loc", prev_hip_loc)


            '''

            head_kp = final_pose[10]

            x_trans = (final_pose[0][0] - prev_hip) * scalepp
            prev_hip = final_pose[0][0]

            final_pose = np.array(final_pose)
            final_pose.T[0::3] -= head_kp[0]
            final_pose.T[1::3] -= head_kp[1]
            final_pose.T[2::3] -= head_kp[2]
            final_pose *= scalepp
            final_pose.T[0::3] += prev_hip_loc[0]
            final_pose.T[1::3] += prev_hip_loc[1]
            final_pose.T[2::3] += prev_hip_loc[2]
            
            final_pose.T[0::3] += x_trans
            prev_hip_loc = final_pose[0]

            '''



            #write_to_csv(final_pose.reshape(1,-1), args, js['image_id'].split('.')[0])
